 ...
 * Values class.
 * 
 * A Values object stores allowed values for a QueryParam of type STRING_LIST. Each allowed value is
 * represented as a {label,value} pair, where the label is used in the UI dropdown, and the value is used
 * in the xml query itself.
 * 
 * @see QueryParam
 */
public class Values {
	 ...
	 *
	 * @param label the UI label to check
		...
		for (int i = 0, length = nodes.getLength(); i < length; ++i) {
			...
			if (XML_ELEMENT == node.getNodeName()) {
				...
				if (labelAttr == null) {
					...
					if (label.isEmpty()) {//check for empty label (use value as label)
						label = value;
					}
					if (isExists(label)) {//check for duplicates
						...
						if ( (defaultAttr != null) && defaultAttr.getNodeValue().equalsIgnoreCase(Boolean.toString(true))) {

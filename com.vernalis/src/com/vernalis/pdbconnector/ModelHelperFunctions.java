...
public class ModelHelperFunctions {
	...
	/**
	 * Constructs the overall xml query string to post to the PDB Advanced Search web service.
	 *
	...
	public static String getXmlQuery(List<QueryOptionModel> queryModels, QueryOptionModel simModel, String conjunction) {
		...
		//outer query string is combined with similarity query (conjunction is always AND).
		return ModelHelperFunctions.getXmlQuery(ModelHelperFunctions.getXmlQuery("",queryModels,conjunction),simModels,Properties.CONJUNCTION_AND);
	}
	...
	public static DataType getDataType(ReportField field) {
		...
		case SMILES:
			return SmilesTypeHelper.INSTANCE.isSmilesAvailable()
		...
		if ( (field == null) || (fieldValue == null) || fieldValue.isEmpty() || fieldValue.equalsIgnoreCase("null")) {//check for missing data
			...
			case INTEGER:
				try {
					...
					PdbConnectorNodeModel.logger.warn("Invalid integer value (" + fieldValue + ") in field: " + field.getColName());
					return DataType.getMissingCell();
				}
			case DOUBLE:
				try {
					...
					PdbConnectorNodeModel.logger.warn("Invalid double value (" + fieldValue + ") in field: " + field.getColName());
					return DataType.getMissingCell();
				}
			case DATE:
				try {
					...
					PdbConnectorNodeModel.logger.debug("Trying simpler " + Properties.YM_FORMAT + " format in field: " + field.getColName());
					try {//try year-month format
						cal.setTime(ymFormat.parse(fieldValue));
						return new DateAndTimeCell(cal.get(Calendar.YEAR),
								cal.get(Calendar.MONTH),1);//force to first day of month
					} catch (ParseException e2) {
						PdbConnectorNodeModel.logger.debug("Trying simpler " + Properties.YEAR_FORMAT + " format in field: " + field.getColName());
						try {//try year-only format
							cal.setTime(yearFormat.parse(fieldValue));
							return new DateAndTimeCell(cal.get(Calendar.YEAR),1,1);//force to first of January
						} catch (ParseException e3) {
							PdbConnectorNodeModel.logger.warn("Invalid date value (" + fieldValue + ") in field: " + field.getColName());
							return DataType.getMissingCell();
						}
					}
				}
			case SMILES:
				return SmilesTypeHelper.INSTANCE.isSmilesAvailable()
							? SmilesTypeHelper.INSTANCE.newInstance(fieldValue)
									: new StringCell(fieldValue);
			case PNG_URL:
				//Concatenate the field value between url prefix and suffix to generate
				//the full image URL
				return new StringCell(Properties.LIGAND_IMG_LOCATION + fieldValue + urlSuffix);
			default:
				throw new IOException("Invalid data type at field=" + field.getColName() + ": value=" + fieldValue);
			}
		}
	}
	 ...
	 *
	 * @param xml the xml query string to post
	...
	public static List<List<String>> getCustomReportCsv(List<String> pdbIds, List<ReportField> selected) throws Exception {
		...
		recordSettings.addQuotePattern(Properties.REPORT_CSV_OPEN_QUOTE, Properties.REPORT_CSV_CLOSE_QUOTE);
	...
	/**
	 * Gets the custom report in XML format from the PDB Custom Report RESTful web service.
	 *
	 * The custom report is returned as a list of list of field values, where the inner list represents a single
	 * report row (one PDB ID), and the outer list represents all PDB IDs.
	 * 
	...
	public static List<List<String>> getCustomReportXml(List<String> pdbIds, List<ReportField> selected) throws Exception {
		...
		if (root == null) {
			...
			throw new IOException("Invalid " + Properties.REPORT_XML_ROOT + " root (" + root.getNodeName() + ")");
		}
		else {

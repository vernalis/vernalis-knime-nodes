...
/**
 * Interface defining the API for native object GC containers
 * 
...
public interface ISWIGObjectGarbageCollector {
	 ...
	 */
	public abstract <T extends Object> T markForCleanup(final T object, final Long wave);

	/**
	 * Overloaded method, which assumes the default 'core' wave (={@code 0}).
	 * 
	 * Registers a SWIG based object that is used within a certain block (wave).
	 * This object must have a delete() method implemented for freeing up
	 * resources later. The cleanup will happen for all registered objects when
	 * the method {@link #cleanupMarkedObjects(int)} is called with the same
	 * wave.
	 * 
	 * @param <T>
	 *            Any class that implements a delete() method to be called to
	 ...
	 * {@link #cleanupMarkedObjects(int)}. Objects registered to more than 1
	 * wave are only cleaned up if this is the only remaining wave they remain
	 * registered to
	 * 
	 * @param wave
	 *            A number that identifies objects registered for a certain
	 *            "wave".
	 */
	public abstract void cleanupMarkedObjects(final Long wave);

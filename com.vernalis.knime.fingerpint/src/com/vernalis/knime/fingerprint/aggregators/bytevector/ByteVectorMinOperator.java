 ...
 * 
 */
public class ByteVectorMinOperator extends AggregationOperator {
	SparseByteVector sbv = null;
	DenseByteVector dbv = null;
	DataType colType;

	public ByteVectorMinOperator() {
		this(new OperatorData("ByteVector Min", "ByteVector Min", "Min", false,
				true, ByteVectorValue.class, false), GlobalSettings.DEFAULT,
				OperatorColumnSettings.DEFAULT_EXCL_MISSING);
	}

	public ByteVectorMinOperator(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		this(new OperatorData("ByteVector Min", "ByteVector Min", "Min", false,
				true, ByteVectorValue.class, false), globalSettings,
				opColSettings);
	}

	public ByteVectorMinOperator(OperatorData operatorData,
	...
	public String getDescription() {
		return "Combines count fingerprints in group using a Min operator"
				+ " in which the minimum count at each"
				+ "byte position is retained in the aggregated result";
	}
	...
	public AggregationOperator createInstance(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		return new ByteVectorMinOperator(getOperatorData(), globalSettings,
				opColSettings);
	}

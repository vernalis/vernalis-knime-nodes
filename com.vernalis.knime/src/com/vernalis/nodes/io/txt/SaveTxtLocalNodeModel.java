...
/**
 * This is the model implementation of SaveTxtLocal.
...
public class SaveTxtLocalNodeModel extends NodeModel {
    ...
    private ColumnRearranger createColumnRearranger(DataTableSpec in) {
        ...
        m_SuccesscolumnName.getStringValue(), BooleanCell.TYPE).createSpec();
        ...
        SingleCellFactory factory = new SingleCellFactory(newColSpec) {
            public DataCell getCell(DataRow row) {
                ...
                if (txtcell.isMissing() || !(txtcell instanceof StringValue)) {
                	//If there is nothing in the stringcell, we return a missing
                ...
                //Here we actually do the meat of the work and save the txt file
                ...
                if (!(FileHelpers.checkContainerFolderExists(pathToFile))){
                	...
                	if (!(FileHelpers.createContainerFolder(pathToFile))){
                		//Folder doesnt exist and for unknown reason we fail to make it
                		return BooleanCell.FALSE;
                	}
                }
                //Now we actually write the file..
                String stringText = ((StringValue)txtcell).getStringValue();
                return (FileHelpers.saveStringToPath(stringText, 
                		pathToFile, m_Overwrite.getBooleanValue())) ? BooleanCell.TRUE : BooleanCell.FALSE;
            }
        };
        c.append(factory);
        return c;
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        if (m_TxtcolumnName.getStringValue() == null) {
            ...
            colIndex =
                    inSpecs[0].findColumnIndex(m_TxtcolumnName.getStringValue());
            if (colIndex < 0) {

...
public class UrlDecodeNodeModel
	...
	private final SettingsModelString charSetMdl =
	...
	protected ColumnRearranger createColumnRearranger(DataTableSpec spec)
		...
		final OutputType outType = OutputType.String_Column;
		// try {
		...
		if (!Charset.availableCharsets()
				.containsKey(charSetMdl.getStringValue())) {
			throw new InvalidSettingsException(
					"Charset '" + charSetMdl.getStringValue()
		...
		rearranger.append(new SingleCellFactory(true, newColSpec) {
			...
			public DataCell getCell(DataRow row) {
				...
				try {
					return cellFactFunc.apply(URLDecoder.decode(input,
							charSetMdl.getStringValue()));

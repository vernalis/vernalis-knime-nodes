...
public class EmptyToMissingCollectionNodeModel
	...
	public EmptyToMissingCollectionNodeModel() {
	...
	protected ColumnRearranger createColumnRearranger(DataTableSpec spec)
		...
		String colNameSuffix =
				replaceInputsMdl.getBooleanValue() ? "" : " (Empty -> Missing)";
			...
			public DataCell[] getCells(DataRow row) {
				...
				for (int col : colIdx) {
					...
					if (!colCell.isMissing()) {
						retVal[cellIndex] =
								((CollectionDataValue) colCell).size() == 0
										? DataType.getMissingCell() : colCell;
					}
					cellIndex++;
				}
				return retVal;

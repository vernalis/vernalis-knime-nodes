...
public class EpmcHelpers {
	 ...
	 * @param queryString The query string for the required query
	 * @param resultType The results type (lite, idlist, lite)
	...
	public static URL buildQueryURL (String queryString, String resultType, int pageNo){
		...
		url+="&page="+pageNo;
		url =url.replace(" ", "%20").replace("\"","%22").replace("[","%5B").replace("]","%5D").replace("+","%20");
		URL url2=null;
		try {
			url2 = new URL(url);
	...
	/**
	 * Function to build the query string for the ePMC from a set of query options
	 * @param Title Title query text
	 * @param Authors Authors query text
	 * @param Affiliations Affiliations query text
	 * @param From From publication date query
	 * @param To To publication date query
	 * @param Journals Journal name(s) query
	 * @param MeSHSubjects  MeSH (Medical Subject Headings) Terms query
	 * @param GeneralQuery  General query - for any other query items with field identifiers
	 * @param sortDate Sort by date?
	...
	public static String buildQueryString (String Title, String Authors, String Affiliations,
		...
		queryString += (Affiliations!=null && !("".equals(Affiliations))) ? addField(Affiliations, "AFF") : "";
		...
		queryString += (MeSHSubjects!=null && !("".equals(MeSHSubjects))) ? addField(MeSHSubjects, "KW") : "";
	 ...
	 * @param category The ePMC query field tag
	...
	/**
	 * Function to mimic the ePMC default year behaviour (which is to 
	 * search to current year + 2
	 * @return The current year + 2
	 */
	public static String defaultYearTo (){
		/*
		 * ePMC searches the year to to the current year +2 by default
		 * if the to parameter is omitted
		 * This function provides a quick way to return that value
		 */
	...
	public static String askEpmc(String queryUrl){
		/*
		 * Run HTTP query with url supplied as string
		 * Simple wrapper to run via URL
		 */
		try{
	 ...
	 * @return XML string containing the query results
	 */
	public static String askEpmc(URL queryUrl) {
		/*
	...
	/**
	 * Very simple XML parser which returns the first occurrence of a field
	 * from an XML string as an XML Data Cell object
	 * @param xml The XML string
	 * @param tag The XML field tag (which does not include < and > characters
	 * @return XML Datacell with the relevant field
	 */
	public static DataCell getCellFromXmlField (String xml, String tag){
		/*
		 * This simple parser assumes only one occurrence of a field in
		 * the xml input (otherwise only the first occurrence will be returned)
		 * A DataCell is returned
		 */
	...
	/**
	 * Very simple XML parser which returns the first occurrence of a field
	 * from an XML string as an XML String
	 * @param xml The XML string
	 * @param tag The XML field tag (which does not include < and > characters
	...
	/**
	 * Function to convert an XML String containing multiple results to a
	 * List of DataCells containing one result per cell 
	 * @param xml The XML string
	 * @param tag The tag identifying the result block
	 * @param wrapperTag The tag to wrap the results in the new xml cell
	 * @return List of XML DataCells
	 */
	public static List<DataCell> getRecordsFromXml (String xml, String tag,
			String wrapperTag){
		...
		for (int i = 1, length = fields.length; i< length; i++){
			if (fields[i].startsWith("<")) {
				...
				newXml = xmlHeader +"<"+wrapperTag+">" + 
						fields[i].split("</"+tag+">")[0]
								+ "</"+wrapperTag+">";
				try {
	 ...
	 * equivalents
	 * @param xml The xml String object

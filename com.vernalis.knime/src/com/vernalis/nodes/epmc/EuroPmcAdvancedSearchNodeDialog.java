...
/**
 * <code>NodeDialog</code> for the "EuroPmcAdvancedSearch" Node.
...
public class EuroPmcAdvancedSearchNodeDialog extends DefaultNodeSettingsPane {
	...
	protected EuroPmcAdvancedSearchNodeDialog() {
		...
		To = new DialogComponentString(
				new SettingsModelString(EuroPmcAdvancedSearchNodeModel.CFG_YEAR_TO, null),
				"To");
		...
		QType = new DialogComponentStringSelection(new SettingsModelString(
    			EuroPmcAdvancedSearchNodeModel.CFG_QUERY_TYPE, "Core"), "Select the query type:", 
    			Arrays.asList("Idlist", "Lite", "Core"));
	   	addDialogComponent (QType);

	   	Sort = new DialogComponentStringSelection(new SettingsModelString(
    			EuroPmcAdvancedSearchNodeModel.CFG_SORT_ORDER, "Date"), "Select the sort order:", 
    			Arrays.asList("Date", "Relevance"));
	   	...
	   	setHorizontalPlacement(false);
	   	//And a query text box - initially populated with some blank lines so as not to make the
	   	//NB we have to use a disabled MultiLineSting as DialogComponentLabel does not support
	   	//multi-line text
	   	m_queryString = new SettingsModelString("queryString","\n\n\n");
	   	m_queryString.setEnabled(false);
	   	queryString = new DialogComponentMultiLineString(m_queryString, "Query (as parsed by ePubMed):");
	...
	protected void doTestQuery() {
		...
		//Now Run it
		String xmlResult = EpmcHelpers.askEpmc(EpmcHelpers.buildQueryURL(queryText,
				((SettingsModelString)QType.getModel()).getStringValue(), 1));
		
		//update the hit counter
		hitCount.setText("Hit Count: " +EpmcHelpers.getStringFromXmlField(xmlResult, "hitCount")
				+ " hits");
		...
		//query = (query ==null) ? "An error occurred connecting to ePMC\nCheck query and try again!":EpmcHelpers.xmlCleanSpecialChars(query);

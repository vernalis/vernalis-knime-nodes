...
public class PdbConnectorNodeModel extends NodeModel {
	 ...
	 *
	 * True limit is 8192, but allow some headroom for integer division rounding errors.
	 */
	...
	protected PdbConnectorNodeModel(PdbConnectorConfig config) {
		...
		if (!config.isOK()) {
			...
			m_ligandImgSize = new SettingsModelString(LIGAND_IMG_SIZE_KEY,m_ligandImgOptions.getDefaultLabel());
			m_conjunction = new SettingsModelString(CONJUNCTION_KEY,Properties.CONJUNCTION_AND_LABEL);
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
		...
		//Divide progress into two subprogress monitors, one for each outport table.
		...
		//determine how many PDB IDs we can process at once without exceeding MAX_URL_LENGTH
		//Each PDB ID requires 5 characters, but have to allow for length of selected column field strings.
		final int CHUNK_SIZE = (MAX_URL_LENGTH - Properties.REPORT_LOCATION.length() - ModelHelperFunctions.getReportColumnsUrl(m_selectedFields).length()) / 5;
		...
		final String urlSuffix = m_ligandImgOptions.isExists(imgSize) ? m_ligandImgOptions.getValue(imgSize) : imgSize;
		row = 0;
		while (!toProcess.isEmpty() && (CHUNK_SIZE > 0)) {
			double progress = 1.0 - (double)toProcess.size() / (double)pdbIds.size();
			exec1.setProgress(progress, "Getting custom report (" + toProcess.size() + "/" + pdbIds.size() + " PDB IDs remaining)");
			...
			List<List<String>> report = ModelHelperFunctions.getCustomReportXml2(nextChunk, m_selectedFields, m_primaryCitationSuffix);
			//List<List<String>> report = ModelHelperFunctions.getCustomReportXml(nextChunk, m_selectedFields);
			//List<List<String>> report = ModelHelperFunctions.getCustomReportCsv(nextChunk, m_selectedFields);
			exec1.checkCanceled();
			for (List<String> recordVals : report) {
				RowKey key = RowKey.createRowKey(row);
				if (recordVals.size() != outputSpec1.getNumColumns()) {
					logger.warn("Invalid record " + recordVals);
					logger.warn("Number of fields expected=" + outputSpec1.getNumColumns()
							+ "; actual=" + recordVals.size());
				}
				else {
					// the cells of the current row, the types of the cells must match
					// the column spec (see above)
	...
	throws InvalidSettingsException {
		if (!m_lastError.isEmpty()) {
			throw new InvalidSettingsException("Error loading query and report definitions from PdbConnectorConfig.xml/.dtd"
					+ " (" + m_lastError + ")");
		}
		DataTableSpec outputSpec0 = createOutputTableSpec0();
		DataTableSpec outputSpec1 = createOutputTableSpec1();
	...
	private DataTableSpec createOutputTableSpec0() {
		...
		for (int i = 0; i < nCols; ++i) {
			allColSpecs[i] = new DataColumnSpecCreator(PDB_COLUMNS[i], StringCell.TYPE).createSpec();
		}
	...
	private DataTableSpec createOutputTableSpec1() {
		//Track selected column names and report URL values, to ensure no duplicates
		...
		for (ReportFieldModel model : m_reportModels) {
			...
			if (model.isSelected()
					&& !columnNames.contains(field.getColName())
					&& !reportValues.contains(field.getValue())) {
				m_selectedFields.add(field);
				columnNames.add(field.getColName());
				reportValues.add(field.getValue());
		...
		for (int i = 0; i < nCols; ++i) {
			...
			allColSpecs[i] = new DataColumnSpecCreator(field.getColName(),ModelHelperFunctions.getDataType(field)).createSpec();
		}
		DataTableSpec retVal = new DataTableSpec(allColSpecs);
		return retVal;
	}

	...
	protected void doConfigure(DataTableSpec dataTableSpec, int portId)
		...
		switch (portId) {
			case 0:
				...
				// Finally, check the column bounds, if they exist, are within
				// the
				// appropriate range
				if (hullNpr1ColIdx >= 0
						&& dataTableSpec.getColumnSpec(hullNpr1ColIdx)
								.getDomain().hasBounds()
						&& ((((DoubleValue) dataTableSpec
								.getColumnSpec(hullNpr1ColIdx).getDomain()
								.getLowerBound()).getDoubleValue() < 0.0)
								|| ((DoubleValue) dataTableSpec
										.getColumnSpec(hullNpr1ColIdx)
				...
				if (hullNpr2ColIdx >= 0
						&& dataTableSpec.getColumnSpec(hullNpr2ColIdx)
								.getDomain().hasBounds()
						&& ((((DoubleValue) dataTableSpec
								.getColumnSpec(hullNpr2ColIdx).getDomain()
								.getLowerBound()).getDoubleValue() < 0.5)
								|| ((DoubleValue) dataTableSpec
										.getColumnSpec(hullNpr2ColIdx)
										.getDomain().getUpperBound())
												.getDoubleValue() > 1.0)) {
					throw new InvalidSettingsException(
				...
				if (m_groupColumnName.getStringValue() != null
					...
					groupColIdx = dataTableSpec.findColumnIndex(
							m_groupColumnName.getStringValue());
				} else {
	...
	protected void addDataSeriesFromInputPort(BufferedDataTable dTable,
		...
		switch (portId) {
			case 0:
				...
				// If the grouping column doesnt provide a color model, then we
				// make
				// one
				// up...
				...
				for (DataRow row : dTable) {
					...
					if (xVal.isMissing() || yVal.isMissing()
						...
						ShapeFactory.Shape rowShape =
								useDefaultShape ? defaultShape
										: shapeHandler.getShape(groupCell);
						// Now retrieve series for the current row...
						if (!retVal.containsKey(ID)) {
							retVal.put(ID, new ConvexHull(ID, rowShape,
									rowColor, m_hullAlpha.getIntValue(),

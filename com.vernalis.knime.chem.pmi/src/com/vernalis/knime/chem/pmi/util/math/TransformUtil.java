...
public class TransformUtil {
	...
	/**
	 * Overloaded method to take a JAMA {@link Matrix} as an argument. Calls
	 * {@link #makeRotationMatrix(double[][])} with the contained matrix
	 * 
	 ...
	 */
	public static double[] makeRotationMatrix(Matrix inertialTensor)
			throws IllegalArgumentException {
		return makeRotationMatrix(inertialTensor.getArray());
	}
	...
	public static double[] makeRotationMatrix(double[][] inertialTensor)
		...
		if (inertialTensor.length != 3 || inertialTensor[0].length != 3) {
			throw new IllegalArgumentException(
					"Inertial Tensor must be a 3x3 matrix");
		}
		...
		double[] r1 = new double[3], r2 = new double[3], v1 = new double[3], v2 = new double[3], v3 = new double[3];
		// int i, j, k, l;
			...
			for (int j = 0; j < 3; ++j) {
				for (int i = 0; i <= j - 1; ++i) {
					...
					if (Math.abs(b) > 0.0) {
						...
						for (int k = 0; k <= i - 1; ++k) {
							atemp = c * inertialTensor[k][i] - s
									* inertialTensor[k][j];
							inertialTensor[k][j] = s * inertialTensor[k][i] + c
									* inertialTensor[k][j];
							inertialTensor[k][i] = atemp;
						}
						for (int k = i + 1; k <= j - 1; ++k) {
							atemp = c * inertialTensor[i][k] - s
									* inertialTensor[k][j];
							inertialTensor[j][k] = s * inertialTensor[i][k] + c
									* inertialTensor[k][j];

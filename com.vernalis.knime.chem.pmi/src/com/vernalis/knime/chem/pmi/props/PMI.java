...
public class PMI {
	...
	public PMI(List<Double> pmis, Double MWt) {
		...
		if (pmis.size() < 3) {
			final double max = pmis.stream().mapToDouble(x -> x.doubleValue()).max()
					.getAsDouble();
			while (pmis.size() < 3) {
				pmis.add(max);
			}
	...
	public static PMI fromPoints(Collection<AbstractPoint<Double>> points) throws RowExecutionException {
		...
		try {
			...
			throw new RowExecutionException("Error calculating centre of gravity!");
		}
		final double[][] inertialTensor =
				AbstractPoint.getInertialTensor(points, cOfGravity);
		final Double a = -1.0 * (inertialTensor[0][0] + inertialTensor[1][1]
				+ inertialTensor[2][2]);
		final Double b = inertialTensor[0][0] * inertialTensor[1][1]
		...
		final Double c = inertialTensor[0][0] * inertialTensor[1][2]
				* inertialTensor[1][2]
						+ inertialTensor[0][1] * inertialTensor[0][1]
								* inertialTensor[2][2]
										+ inertialTensor[0][2] * inertialTensor[0][2]
														...
														- 2 * inertialTensor[0][1] * inertialTensor[1][2]
																* inertialTensor[0][2]
																		- inertialTensor[0][0] * inertialTensor[1][1]
																				* inertialTensor[2][2];
		final Cubic cubic = new Cubic(a, b, c);

 ...
 * 
 * A QueryOptionModel defines the settings models for a single query option. It
 * contains:
 * <UL>
 * <LI>The query option (and the list of child query parameters) that are
 * represented by the model</LI>
...
public class QueryOptionModel {
	 ...
	 *
	 * The placeholders (%ARG%) in the query string are replaced with the
	 * current values of the query parameters, as defined by the relevant
	 * settings models.
	 * 
			...
			while (paramIter.hasNext() && modelsIter.hasNext()) {
				...
				switch (param.getType()) {
					...
					case STRING_LIST:
						assert models.size() == 1;
						arg = "%ARG" + (argNum++) + "%";
						// For String_Lists we need to retrieve the underlying
						// value
					...
					case INTEGER_RANGE:
						...
						for (int i = 0, numModels =
							...
							arg = "%ARG" + (argNum++) + "%";
							retVal = retVal.replaceAll(arg, Integer.toString(
					...
					case DOUBLE_RANGE:
						...
						for (int i = 0, numModels =
								models.size(); i < numModels; ++i) {
							arg = "%ARG" + (argNum++) + "%";
							retVal = retVal.replaceAll(arg, Double.toString(
					...
					case STRING_COND:
						assert (models.size() == 1)
								&& (subQueries.size() == 1); {
						// Perform subQuery replacement on the string value
						// entered
						// Subquery is only active if a non-empty string is
						// entered
					...
					case INTEGER_RANGE_COND:
						assert (models.size() == 2)
								&& (subQueries.size() == 2); {
						// Perform subQuery replacements on the min and max
						// values
						// entered.
						// Subqueries are only active if the value entered is gt
						// min
	 ...
	 *
	 * Calls {@link SettingsModel#saveSettingsTo(NodeSettingsWO)} on each
	 ...
	 * 
	 * Calls {@link SettingsModel#loadSettingsFrom(NodeSettingsRO)} on each
	 * underlying settings model.
	 *
	 * @param settings
	 *            the settings
	 ...
	 * 
	 * Calls {@link SettingsModel#validateSettings(NodeSettingsRO)} on each
	 * underlying settings model.
	 * 
	 * @param settings
	 *            the settings

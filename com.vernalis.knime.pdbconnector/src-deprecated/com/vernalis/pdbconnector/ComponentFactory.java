 ...
 * @see QueryParam
 * @see ReportField2
 * @deprecated Use {@link ComponentFactory2}
 */
@Deprecated
public class ComponentFactory {
	...
	public static SettingsModelBoolean createSelectionSettingsModel(QueryOption query) {
		return (query != null)
				? new SettingsModelBoolean(query.getId() + ".SELECTED", query.getDefault()) : null;
	}
	...
	public static DialogComponentBoolean createSelectionDialogComponent(QueryOption query) {
		return (query != null)
				? new DialogComponentBoolean(createSelectionSettingsModel(query), "Selected")
				: null;
	}
	...
	public static DialogComponentBoolean createSelectionDialogComponent(ReportField field) {
		return (field != null)
				? new DialogComponentBoolean(createSelectionSettingsModel(field), field.getLabel())
				: null;
	}
	...
	public static List<SettingsModel> createSettingsModels(QueryParam param, boolean isEnabled) {
		...
		if (param != null) {
			...
			case STRING_COND:
				...
				break;
			case INTEGER:
				retVal.add(new SettingsModelIntegerBounded(param.getId(), (int) param.getDefault(),
						(int) param.getMin(), (int) param.getMax()));
				break;
			case DOUBLE:
				retVal.add(new SettingsModelDoubleBounded(param.getId(), param.getDefault(),
						param.getMin(), param.getMax()));
				break;
			case INTEGER_RANGE:
			case INTEGER_RANGE_COND:
				retVal.add(new SettingsModelIntegerBounded(param.getId() + ".MIN",
						(int) param.getMin(), (int) param.getMin(), (int) param.getMax()));
				retVal.add(new SettingsModelIntegerBounded(param.getId() + ".MAX",
						(int) param.getMax(), (int) param.getMin(), (int) param.getMax()));
				break;
			case DOUBLE_RANGE:
			case DOUBLE_RANGE_COND:
				retVal.add(new SettingsModelDoubleBounded(param.getId() + ".MIN", param.getMin(),
						param.getMin(), param.getMax()));
				retVal.add(new SettingsModelDoubleBounded(param.getId() + ".MAX", param.getMax(),
						param.getMin(), param.getMax()));
				break;
			case STRING_LIST:
				retVal.add(new SettingsModelString(param.getId(),
			...
			case DATE:
				...
				retVal.add(
						new SettingsModelIntegerBounded(param.getId() + ".YYYY", year, 1900, year));
				retVal.add(new SettingsModelIntegerBounded(param.getId() + ".MM",
						now.get(Calendar.MONTH) + 1, 1, 12));
				retVal.add(new SettingsModelIntegerBounded(param.getId() + ".DD",
	...
	public static List<DialogComponent> createDialogComponents(QueryParam param,
		...
		if (param != null) {
			...
			case STRING_COND:
				...
				retVal.add(new DialogComponentString((SettingsModelString) models.get(0),
						param.getLabel(), false, param.getWidth()));
				break;
			case INTEGER:
				assert models.size() == 1;
				retVal.add(new DialogComponentNumber((SettingsModelIntegerBounded) models.get(0),
						param.getLabel(), 1, param.getWidth()));
				break;
			case DOUBLE:
				assert models.size() == 1;
				retVal.add(new DialogComponentNumber((SettingsModelDoubleBounded) models.get(0),
			...
			case INTEGER_RANGE_COND:
				...
				retVal.add(new DialogComponentNumber((SettingsModelIntegerBounded) models.get(1),
						"max", 1, param.getWidth()));
				break;
			case DOUBLE_RANGE:
			case DOUBLE_RANGE_COND:
				assert models.size() == 2;
				...
				retVal.add(new DialogComponentNumber((SettingsModelDoubleBounded) models.get(1),
						"max", 1.0, param.getWidth()));
				break;
			case STRING_LIST:
				assert models.size() == 1;
			...
			case DATE:
				...
				DialogComponentNumber dayDlg = new DialogComponentNumber(
						(SettingsModelIntegerBounded) models.get(2), "DD", 1);
				// remove thousands separator from year fields and set leading
			...
			case BIG_STRING:
				assert models.size() == 1;
				retVal.add(new DialogComponentMultiLineString((SettingsModelString) models.get(0),
						param.getLabel(), false, param.getWidth(),
		...
		if (param != null) {
			...
			case STRING_LIST:
				...
				((SettingsModelString) dlgs.get(0).getModel())
						.setStringValue(param.getValues().getDefaultLabel());
				break;
			case DATE:
				assert dlgs.size() == 3;
				Calendar now = Calendar.getInstance();

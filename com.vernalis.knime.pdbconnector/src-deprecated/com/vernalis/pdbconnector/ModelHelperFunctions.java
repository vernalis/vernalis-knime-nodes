 ...
 * @deprecated Use ModelHelperFunctions2
 */
@Deprecated
public class ModelHelperFunctions {
	...
	public static List<String> postQuery(final String xml) throws IOException {
		...
		while ((line = rd.readLine()) != null) {
			...
			if ("null".equals(line)) {
				rd.close();
				throw new IOException("PDB RESTful Query Webservice returned response 'null'");
			}
	 ...
	 * @param pdbIds
	 *            the list of PDB IDs to generate report for
	 * @param m_selectedFields
	 *            the selected report fields to include in the report
	 * @param m_primaryCitationSuffix
	...
	public static List<List<String>> postCustomReportXml2(List<String> pdbIds,
			List<ReportField> m_selectedFields, ReportField m_primaryCitationSuffix)
			throws Exception {

		// Sort out the data to POST to the service
		StringBuffer buf = new StringBuffer(ModelHelperFunctions.getPdbIdUrl(pdbIds));
		buf.append(ModelHelperFunctions.getReportColumnsUrl(m_selectedFields));
		buf.append(Properties.REPORT_XML_URL);
		if (m_primaryCitationSuffix != null) {
			buf.append(m_primaryCitationSuffix.getValue());
		}
	 ...
	 * @return the URL string (comma-separated values)
	 */
	public static String getReportColumnsUrl(final List<ReportField> m_selectedFields) {
		StringBuffer buf = new StringBuffer(Properties.REPORT_COLUMNS_URL);
		boolean isFirst = true;
		for (ReportField field : m_selectedFields) {
			if (!isFirst) {
				buf.append(",");
			}

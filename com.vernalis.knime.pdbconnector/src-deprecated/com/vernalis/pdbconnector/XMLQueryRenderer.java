...
public abstract class XMLQueryRenderer {
	...
	static {
		...
		for (QueryCategory qCat : PdbConnectorConfig2.getInstance()
				.getQueryCategories()) {
	...
	public static String XMLtoQuery(String xml) {
			...
			for (QueryOption qOpt : allOptions) {
				if (qOpt.getQueryString().indexOf(type) >= 0) {
					...
					StringBuilder replacement =
							new StringBuilder(qOpt.getLabel()).append(":\n");
					int qParamIndex = 0;
					...
					for (int g = 1; g <= matcher.groupCount(); g++) {
						...
						if (qParam.getType() == eType.STRING_LIST) {
							...
							// String_COND type
							if (qParamIndex + 1 < qParams.size()
									&& qParams.get(qParamIndex + 1)
											.getType() == eType.STRING_COND) {
								// We may need this, or may not...
								optionalFilter = "\t" + qParam.getLabel() + ": "
										+ matcher.group(g) + " \"";
							} else {
								replacement.append("\t")
										.append(qParam.getLabel())
										.append(": \"");
								replacement.append(qParam.getValues()
										.lookupKeyForValue((matcher.group(g))));
								replacement.append("\"");
							...
							qParamIndex++;
						} else if (qParam.getTypeAsString().toUpperCase()
								.indexOf("_COND") >= 0) {
							if (qParam.getTypeAsString().toUpperCase()
									.indexOf("RANGE") >= 0) {
								...
								.indexOf("RANGE") >= 0) {
							replacement.append("\t").append(qParam.getLabel())
									.append(": ");
		...
		while (retValStr.indexOf(linebreak + linebreak + linebreak) >= 0) {
			retValStr = retValStr.replace(linebreak + linebreak + linebreak,
					linebreak + linebreak);
		}
		retValStr = retValStr.replaceAll(linebreak + linebreak + "(\t*\\))",
				linebreak + "$1");
		return retValStr;
	}

	public static String replaceNthOccurence(String inputStr,
			String searchString, String replacementString, int N) {

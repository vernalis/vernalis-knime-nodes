...
import com.vernalis.pdbconnector.nodes.pdbconnector.AbstractPdbConnectorNodeModel;
...
public class ReportOptionsDialog2 extends JPanel {
	...
	static final NodeLogger logger =
			NodeLogger.getLogger(ReportOptionsDialog2.class);
	...
	public ReportOptionsDialog2(PdbConnectorConfig2 config) {
		super();
		...
		m_comboBox.setRenderer(new ListCellRenderer() {
			...
			public Component getListCellRendererComponent(JList list,
				...
				if (value instanceof StandardCategory) {// disable the category
					...
					c = r.getListCellRendererComponent(list, label, index,
							false, false);
					c.setEnabled(false);
		...
		m_comboBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (item instanceof StandardReport) {
					...
					for (ReportCategoryDialog2 dlg : m_reportDlgs) {
		...
		for (ReportCategory2 category : categories) {
			if (!category.isHidden()) {
				ReportCategoryDialog2 reportDlg =
						new ReportCategoryDialog2(category);
				super.add(reportDlg);
	...
	public final void loadSettingsFrom(final NodeSettingsRO settings,
		...
		assert specs != null;
		for (ReportCategoryDialog2 reportDialog : m_reportDlgs) {
			reportDialog.loadSettingsFrom(settings, specs);
		}
		try {
			updateComponent(settings
					.getString(AbstractPdbConnectorNodeModel.STD_REPORT_KEY));
		} catch (InvalidSettingsException e) {
			logger.warn("Error loading string for key "
					+ AbstractPdbConnectorNodeModel.STD_REPORT_KEY);
	...
	public final void saveSettingsTo(final NodeSettingsWO settings)
		...
		if (o instanceof StandardReport) {
			settings.addString(AbstractPdbConnectorNodeModel.STD_REPORT_KEY,
					((StandardReport) o).getId());
		}
	...
	private boolean setSelectedReport(String reportId) {
		...
		if (reportId != null) {
			for (int i = 0, length = m_comboBox.getItemCount(); (i < length)
					&& !retVal; i++) {

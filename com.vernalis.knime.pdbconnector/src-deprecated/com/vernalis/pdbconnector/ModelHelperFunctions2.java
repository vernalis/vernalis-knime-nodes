...
public class ModelHelperFunctions2 {
				...
				if (fieldValues[i] == null || fieldValues[i].isEmpty()) {
					...
					switch (field.getType()) {
						...
						case DATE:
							try {
								...
								try {// try year-month format
									...
									cells[i] = new DateAndTimeCell(
											cal.get(Calendar.YEAR),
											cal.get(Calendar.MONTH), 1);// force
																		// to
																		// first
																		// day
																		// of
						...
						case SMILES:
							...
							break;
						case PNG_URL:
							// Concatenate the field value between url prefix
							// and
							// suffix to
							// generate
							// the full image URL
							cells[i] = new StringCell(
									Properties.LIGAND_IMG_LOCATION
			...
			switch (field.getType()) {
				...
				case INTEGER:
					try {
						...
						exception.addWarning(
								"Invalid integer value (" + fieldValue
				...
				case DOUBLE:
					try {
						...
						exception.addWarning(
								"Invalid double value (" + fieldValue
										+ ") in field: " + field.getColName());
						retVal = DataType.getMissingCell();
					}
					break;
				case DATE:
					try {
						...
						try {// try year-month format
							...
							exception.addInfo(
									"Trying simpler " + Properties.YEAR_FORMAT
											+ " format in field: "
											+ field.getColName());
							try {// try year-only format
								cal.setTime(yearFormat.parse(fieldValue));
								retVal = new DateAndTimeCell(
										cal.get(Calendar.YEAR), 1, 1);// force
																		// to
																		// first
																		// of
				...
				case SMILES:
					...
					break;
				case PNG_URL:
					// Concatenate the field value between url prefix and suffix
					// to
					// generate
					// the full image URL
	...
	private static Map<String, List<List<String>>>
			manuallyParseXMLToMap(List<String> report, ExecutionMonitor exec)
					throws IOException, CanceledExecutionException {

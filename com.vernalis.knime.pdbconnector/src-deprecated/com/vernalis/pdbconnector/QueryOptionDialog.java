...
public class QueryOptionDialog extends JPanel {
	...
	public QueryOptionDialog(final QueryOption queryOption) {
		...
		super.setBorder(BorderFactory.createTitledBorder(
				BorderFactory.createEtchedBorder(), m_queryOption.getLabel()));
		m_selected =
			...
			public void stateChanged(final ChangeEvent e) {
				...
				for (final List<DialogComponent> dlgs : m_dlgs) {
					for (final DialogComponent dlg : dlgs) {
						...
						// enabled/disabled...
						for (final Component com : ComponentFactory2
		...
		while (iter.hasNext()) {
			...
			int ipadx =
					gridWidth * Properties.QUERY_LAYOUT_COL_WIDTH - panelWidth;
			...
			cDlg.gridy = iRow;
			cDlg.gridwidth =
					(isTooWide || isOrphan) ? GridBagConstraints.REMAINDER
							: gridWidth;
			cDlg.gridheight = 1;
	...
	public void resetParams() {
		...
		while (paramIter.hasNext() && dlgsIter.hasNext()) {
			...
			ComponentFactory2.resetDialogComponents(param, dlgs);
		}
	}

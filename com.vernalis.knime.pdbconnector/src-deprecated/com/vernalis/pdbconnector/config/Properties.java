 ...
 * Properties class.
 * 
 * Contains static instances of all string and integer global properties.
 * Property values may be used either to control model behaviour or to control
 * UI dialog layout.
 * 
 * Property values are initialized from configuration XML file by
 * PdbConnectorConfig, or to a suitable internal (hard-coded) default value if
...
public class Properties {
	...
	/**
	 * Number of columns in Report Options dialog (excluding category checkbox
	 * column).
	 */
	...
	static {
		...
		COMPOSITE_END =
				config.getProperty("COMPOSITE_END", "</orgPdbCompositeQuery>");
		REFINEMENT_START =
				config.getProperty("REFINEMENT_START", "<queryRefinement>");
		REFINEMENT_END =
				config.getProperty("REFINEMENT_END", "</queryRefinement>");
		LEVEL_START =
				config.getProperty("LEVEL_START", "<queryRefinementLevel>");
		LEVEL_END = config.getProperty("LEVEL_END", "</queryRefinementLevel>");
		CONJUNCTION_AND = config.getProperty("CONJUNCTION_AND",
				"<conjunctionType>and</conjunctionType>");
		CONJUNCTION_OR = config.getProperty("CONJUNCTION_OR",
		...
		REPORT_COLUMNS_URL = config.getProperty("REPORT_COLUMNS_URL",
				"&customReportColumns=");
		REPORT_CSV_URL = config.getProperty("REPORT_CSV_URL", "&format=csv");
		REPORT_CSV_LINE_DELIM =
				config.getProperty("REPORT_CSV_LINE_DELIM", "<br />");
		REPORT_CSV_RECORD_DELIM =
				config.getProperty("REPORT_CSV_RECORD_DELIM", ",");
		REPORT_CSV_OPEN_QUOTE =
				config.getProperty("REPORT_CSV_OPEN_QUOTE", "\"");
		REPORT_CSV_CLOSE_QUOTE =
				config.getProperty("REPORT_CSV_CLOSE_QUOTE", "\"");
		...
		// UI labels
		CONJUNCTION_AND_LABEL =
				config.getProperty("CONJUNCTION_AND_LABEL", "and");
		CONJUNCTION_OR_LABEL = config.getProperty("CONJUNCTION_OR_LABEL", "or");
		// UI layout (integers)
		REPORT_LAYOUT_COLUMNS =
				config.getPropertyAsInt("REPORT_LAYOUT_COLUMNS", 2);
		REPORT_LAYOUT_COL_WIDTH =
				config.getPropertyAsInt("REPORT_LAYOUT_COL_WIDTH", 250);
		QUERY_PRM_STRING_WIDTH =
				config.getPropertyAsInt("QUERY_PRM_STRING_WIDTH", 15);
		QUERY_PRM_INTEGER_WIDTH =
				config.getPropertyAsInt("QUERY_PRM_INTEGER_WIDTH", 5);
		QUERY_PRM_DOUBLE_WIDTH =
				config.getPropertyAsInt("QUERY_PRM_DOUBLE_WIDTH", 5);
		QUERY_PRM_BIGSTRING_WIDTH =
				config.getPropertyAsInt("QUERY_PRM_BIGSTRING_WIDTH", 40);
		QUERY_PRM_BIGSTRING_ROWS =
				config.getPropertyAsInt("QUERY_PRM_BIGSTRING_ROWS", 3);
		QUERY_LAYOUT_COLUMNS =
				config.getPropertyAsInt("QUERY_LAYOUT_COLUMNS", 3);
		QUERY_LAYOUT_COL_WIDTH =
				config.getPropertyAsInt("QUERY_LAYOUT_COL_WIDTH", 200);
	}
}

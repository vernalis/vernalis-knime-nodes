...
public class XmlDataParsingException extends Exception {
	...
	public boolean hasLevelMessage(Level level) {
		return errors.stream().anyMatch(a -> ((StringValue) cell)
				.getStringValue().startsWith(level.toString()));
	}
	...
	public List<DataCell> getErrors(Level level) {
		return errors.stream()
				.filter(x -> ((StringValue) x).getStringValue()
						.startsWith(level.toString()))

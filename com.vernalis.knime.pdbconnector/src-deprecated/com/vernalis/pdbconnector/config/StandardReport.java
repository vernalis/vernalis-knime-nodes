 ...
 * 
 * A StandardReport represents a pre-defined set of report fields that
 * constitute a standard report. This is in contrast to a custom report where
 * the user has full control over the field selection.
 * 
 * Notes:
 * <OL>
 * <LI>The constituent report fields are not stored as part of the
 * StandardReport object. Rather, the StandardReport acts as a trigger to select
 * or deselect the individual ReportField objects stored in each
 * ReportCategory.</LI>
 * <LI>A StandardReport (for example Ligand) may not select all of the report
 * fields in the ReportCategory of the same name, and may include fields from
 * other report categories.</LI>
 * </OL>
 *
 * @see ReportField2
 * @see ReportField2#isTriggered(StandardReport)
 * @see ReportCategory2
	...
	private void initFromXML(Node node) throws ConfigException {
		if (node == null) {
			...
			if (id == null) {
				...
				m_isDefault = defaultAttr.getNodeValue()

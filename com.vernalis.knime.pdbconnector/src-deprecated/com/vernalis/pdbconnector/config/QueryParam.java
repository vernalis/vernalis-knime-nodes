	...
	private void initFromXML(final Node node) throws ConfigException {
		if (node == null) {
			...
			if (id == null) {
				...
				throw new ConfigException("Missing " + XML_ATTR_WIDTH
						+ " attribute in " + XML_ELEMENT);
			} else if (min == null) {
				throw new ConfigException("Missing " + XML_ATTR_MIN
						+ " attribute in " + XML_ELEMENT);
			} else if (max == null) {
				throw new ConfigException("Missing " + XML_ATTR_MAX
						+ " attribute in " + XML_ELEMENT);
				...
				try {
					...
					throw new ConfigException("Invalid " + XML_ATTR_WIDTH
							+ " attribute (" + width.getNodeValue() + ") in "
							+ XML_ELEMENT);
				}
				...
				try {
					...
					throw new ConfigException("Invalid " + XML_ATTR_MIN
							+ " attribute (" + min.getNodeValue() + ") in "
							+ XML_ELEMENT);
				}
				try {
					m_max = Double.parseDouble(max.getNodeValue());
					if (m_max < m_min) {
						throw new ConfigException("Out of range " + XML_ATTR_MAX
								+ " attribute (" + m_max + ") in " + XML_ELEMENT
								+ " (" + XML_ATTR_MIN + "=" + m_min + ")");
					}
				} catch (NumberFormatException e) {
					throw new ConfigException("Invalid " + XML_ATTR_MAX
							+ " attribute (" + max.getNodeValue() + ") in "
							+ XML_ELEMENT);
				}
				try {
					m_default = Double.parseDouble(defaultAttr.getNodeValue());
					if (m_default < m_min) {
						throw new ConfigException("Out of range "
								+ XML_ATTR_DEFAULT + " attribute (" + m_default
								+ ") in " + XML_ELEMENT + " (" + XML_ATTR_MIN
								+ "=" + m_min + ")");
					} else if (m_default > m_max) {
						throw new ConfigException("Out of range "
								+ XML_ATTR_DEFAULT + " attribute (" + m_default
								+ ") in " + XML_ELEMENT + " (" + XML_ATTR_MAX
								+ "=" + m_max + ")");
					}
				} catch (NumberFormatException e) {
					throw new ConfigException("Invalid " + XML_ATTR_DEFAULT
							+ " attribute (" + defaultAttr.getNodeValue()
							+ ") in " + XML_ELEMENT);
				}
					...
					case STRING_LIST:
						...
						if (m_values.getLabels().isEmpty()) {
							throw new ConfigException(
									"Missing " + Values.XML_ELEMENT
											+ " elements in " + XML_ELEMENT);
						}
						break;
					case STRING_COND:// load conditional subquery strings
										// (should be
										// one or two, depending on type)
					case INTEGER_RANGE_COND:
					case DOUBLE_RANGE_COND:
						int numExpected =
								(m_type == QueryParam.eType.STRING_COND) ? 1
										: 2;
						NodeList children = node.getChildNodes();
						for (int i = 0, length =
								children.getLength(); i < length; ++i) {
							Node child = children.item(i);
							if (XML_ELEMENT_QUERY_STRING == child
									.getNodeName()) {
								m_queryStrings.add(child.getTextContent());
							}
						}
						if (m_queryStrings.size() != numExpected) {
							throw new ConfigException("Incorrect number of "
									+ XML_ELEMENT_QUERY_STRING
									+ " child elements in " + XML_ELEMENT
									+ " (expected=" + numExpected + ", actual="
									+ m_queryStrings.size() + ")");
						}
						break;
					default:

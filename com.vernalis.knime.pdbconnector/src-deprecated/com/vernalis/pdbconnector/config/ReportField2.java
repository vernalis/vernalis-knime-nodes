 ...
 * <LI>Triggers (other report fields, report categories, or standard reports
 * that will trigger the selection of this field)</LI>
 * </UL>
 *
 * SDR 07/Jul/2016 - Added support for ListCells, indicated by presence of
 * optional 'deliminator' attribute in XML
 * 
 * @see ReportCategory2
 * @see StandardReport
 */
@Deprecated
public class ReportField2 {
	...
	/** XML attribute name for ReportField deliminators */
	...
	private ReportCategory2 m_parent;
	private String m_id;
	 ...
	 */
	public static eType string2Type(final String strType)
			throws ConfigException {
		if (strType.equalsIgnoreCase("string")) {
			...
			throw new ConfigException("Invalid " + XML_ATTR_TYPE
					+ " attribute (" + strType + ")");
		}
	}
	 ...
	 *
	 * @param parent
	 *            the parent ReportCategory
	 * @param node
	 *            the XML node
	 * @throws ConfigException
	 *             if any parse errors.
	 */
	public ReportField2(final ReportCategory2 parent, final Node node)
			throws ConfigException {
		m_parent = parent;
		initFromXML(node);
	}
	 ...
	 */
	public final ReportCategory2 getParent() {
		return m_parent;
	}
	...
	/**
	 * Checks if this is the hidden report field controlling the primary
	 * citation suffix.
	 *
	 * The primary citation suffix field is defined by the special name
	 * "PRIMARY_CITATION_SUFFIX". This hidden pseudo field is triggered
	 * automatically by selection of any field in the primary citation category,
	 * and should not be user-selectable in the UI.
	 *
	 * Unlike all other report fields, the primary citation suffix does not
	 * represent a column in the KNIME output table. Rather, it is a modifier of
	 * the behaviour of the citation report fields. Hence, the string value
	 * represented by this field should be appended to the custom report URL,
	...
	/**
	 * @return Whether the field is a list
	 */
	...
	/**
	 * @return A deliminator string if the field is a list, otherwise
	 *         {@code null}
	 */
	public final String getListDeliminator() {
		return m_delim;
	}
	 ...
	 *
	 * Returns true if the other ReportField (or the parent category of the
	 * other ReportField) is in the trigger list for this field.
	 *
	 * @param field
	 *            the field
	 * @return true, if is triggered
	 */
	public boolean isTriggered(final ReportField2 field) {
		return m_trigger.contains(field.getParent().getId())
				|| m_trigger.contains(field.getId());
	}

	/**
	 * Checks if is triggered by a StandardReport.
	 *
	 * Returns true if this field should be selected as part of a
	 * StandardReport.
	 *
	 * @param report
	 *            the report
	...
	private void initFromXML(final Node node) throws ConfigException {
		...
		if (node == null) {
			...
			if (id == null) {
				...
				throw new ConfigException("Missing " + XML_ATTR_TYPE
						+ " attribute in " + XML_ELEMENT);
			} else if (label == null) {
				throw new ConfigException("Missing " + XML_ATTR_LABEL
						+ " attribute in " + XML_ELEMENT);
			} else if (colName == null) {
				throw new ConfigException("Missing " + XML_ATTR_COLNAME
						+ " attribute in " + XML_ELEMENT);
				...
				Node delimAttr = attr.getNamedItem(XML_ATTR_DELIMINATOR);
				if (delimAttr != null) {
					m_delim = delimAttr.getNodeValue();
					if (m_delim.isEmpty()) {
						m_delim = null;
					} else {
						m_isList = true;

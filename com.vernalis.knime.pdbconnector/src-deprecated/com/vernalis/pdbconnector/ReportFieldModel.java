 ...
 *
 * A ReportFieldModel defines the Boolean settings model for a single report
 * field.
 * 
 * @deprecated User {@link ReportFieldModel2}
 */
@Deprecated
public class ReportFieldModel {
	 ...
	 *
	 * The selection state of this model is set to true if any of the report
	 * field models passed in is a trigger for this field AND is selected.
	 *
	 * @param models
	 *            the report field models
	 * @return new selection state
	 */
	public boolean applyTrigger(final List<ReportFieldModel> models) {
		m_selected.setBooleanValue(m_field.getDefault());// reset to initial
															// state (mandatory
															// or optional)
		Iterator<ReportFieldModel> iter = models.iterator();
		while (iter.hasNext() && !m_selected.getBooleanValue()) {
			ReportFieldModel model = iter.next();
	 ...
	 *
	 * Overrides the selection state of this model, if a non-customizable
	 * standard report is applied.
	 *
	 * @param stdReport

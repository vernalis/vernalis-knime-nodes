...
public class RCSBsDownloadNodeModel extends NodeModel {
	...
	private DataRow buildRow(final String pdbid, long currentRowID, final ExecutionContext exec)
		...
		if (m_PDB.getBooleanValue()) {
			r = FileHelpers.readURLToString(PDBIOHelperFunctions.createRCSBUrl(pdbid, "PDB", true));
			if (r != null) {
		...
		if (m_CIF.getBooleanValue()) {
			r = FileHelpers.readURLToString(PDBIOHelperFunctions.createRCSBUrl(pdbid, "cif", true));
			if (r != null) {
		...
		if (m_SF.getBooleanValue()) {
			r = FileHelpers.readURLToString(PDBIOHelperFunctions.createRCSBUrl(pdbid, "sf", true));
			if (r != null) {
				row[i] = new StringCell(r);
			}
			i++;
		...
		if (m_PDBML.getBooleanValue()) {
			r = FileHelpers
					.readURLToString(PDBIOHelperFunctions.createRCSBUrl(pdbid, "PDBML", true));
			if (r != null) {
				try {
		...
		if (m_FASTA.getBooleanValue()) {
			r = FileHelpers.readURLToString(PDBIOHelperFunctions.createRCSBUrl(pdbid, "FASTA"));
			if (r != null) {
				row[i] = new StringCell(r);
			}
	...
	private static DataColumnSpec[] createDataColumnSpec() {
		/*
		 * Create the column specs for the output table based on the specified
		 * column selections
		 */
		m_colCnt = countCols();
	...
	private static int countCols() {
		// Count the new columns to add NB - We start at 1 because we always add
		// the PDB ID as a column
		int i = 1;

...
/**
 * <code>NodeDialog</code> for the "RCSBGrabber" Node. Node to allow download of
 * multiple RCSB PDB filetypes from a column of RCSB Structure IDs
 */
public class RCSBsDownload2NodeDialog extends DefaultNodeSettingsPane {

	/**
	 * New pane for configuring the RCSBGrabber node.
	 */
	protected RCSBsDownload2NodeDialog() {
		super();
		createNewGroup("PDB ID Column");
		addDialogComponent(new DialogComponentString(createPDBIDsModel(), "Enter a valid PDB ID:"));
	...
	/**
	 * @return The settings model containing the PDB ID(s)
	 */
	static SettingsModelString createPDBIDsModel() {
		return new SettingsModelString("PDB_ID", null);
	}

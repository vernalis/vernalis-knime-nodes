...
public class CombineQueryNodeModel extends AbstractXMLQueryProviderNodeModel {
	...
	protected final SettingsModelString m_leftQueryNameMdl = createLeftVarNameModel();
	protected final SettingsModelString m_rightQueryNameMdl = createRightVarNameModel();
	protected final SettingsModelString m_conjMdl = createConjunctionModel();
	protected final SettingsModelString m_outVarNameMdl = createQueryNameModel();
	...
	protected PortObjectSpec[] configure(PortObjectSpec[] inSpecs) throws InvalidSettingsException {
		...
		for (Entry<String, FlowVariable> ent : fVars.entrySet()) {
			if (ent.getValue().getType() == Type.STRING && !ent.getValue().isGlobalConstant()) {
				strVars.add(ent.getKey());
			}
		}
		if (strVars.size() < 2) {
			throw new InvalidSettingsException("At least two string flow variables are required");
		}
		int nextGuess = strVars.size() - 1;

		if (m_rightQueryNameMdl.getStringValue() == null
				|| m_rightQueryNameMdl.getStringValue().isEmpty()) {
			logger.info("Guessing " + strVars.get(nextGuess) + " for right query variable name");
		...
		if (m_leftQueryNameMdl.getStringValue() == null
			...
			logger.info("Guessing " + strVars.get(nextGuess) + " for left query variable name");
		...
		if (!strVars.contains(m_leftQueryNameMdl.getStringValue())) {
			throw new InvalidSettingsException("Left query variable ("
					+ m_leftQueryNameMdl.getStringValue() + ") is not available");
		}
		if (!strVars.contains(m_rightQueryNameMdl.getStringValue())) {
			throw new InvalidSettingsException("Right query variable ("
					+ m_rightQueryNameMdl.getStringValue() + ") is not available");
		}

		if (m_rightQueryNameMdl.getStringValue().equals(m_leftQueryNameMdl.getStringValue())) {
			throw new InvalidSettingsException("Both variables cannot be the same!");
		}
	...
	protected PortObject[] execute(PortObject[] inObjects, ExecutionContext exec) throws Exception {
		String conjunction = m_conjMdl.getStringValue().equals("AND") ? Properties.CONJUNCTION_AND
				: Properties.CONJUNCTION_OR;
		pushFlowVariableString(m_outVarNameMdl.getStringValue(),
				ModelHelperFunctions2.combineQueries(
						peekFlowVariableString(m_leftQueryNameMdl.getStringValue()),
						peekFlowVariableString(m_rightQueryNameMdl.getStringValue()), conjunction));

...
public class CombineQueryNodeDialog extends DefaultNodeSettingsPane {
	/** The logger instance */
	private static final NodeLogger LOGGER = NodeLogger.getLogger(CombineQueryNodeDialog.class);
	...
	public CombineQueryNodeDialog() {
		...
		m_leftVarSelector = new DialogComponentFlowVariableNameSelection(m_leftVarMdl,
				"First Query", strFlowVars, FlowVariable.Type.STRING);
		addDialogComponent(m_leftVarSelector);
		addDialogComponent(
				new DialogComponentStringSelection(createConjunctionModel(), "", "AND", "OR"));
		m_rightVarMdl = createRightVarNameModel();
		m_rightVarSelector = new DialogComponentFlowVariableNameSelection(m_rightVarMdl,
				"Second Query", strFlowVars, FlowVariable.Type.STRING);
		addDialogComponent(m_rightVarSelector);
		setHorizontalPlacement(false);
		addDialogComponent(
				new DialogComponentString(createQueryNameModel(), "Output Query Name", true, 20));
	}
	...
	protected List<FlowVariable> getStrFlowVars() {
		...
		for (FlowVariable fVar : getAvailableFlowVariables().values()) {
			if (!fVar.isGlobalConstant() && fVar.getType() == FlowVariable.Type.STRING) {
	...
	public void loadAdditionalSettingsFrom(final NodeSettingsRO settings,
		...
		try {
			...
			lVar = ((SettingsModelString) m_leftVarMdl.createCloneWithValidatedValue(settings))
					.getStringValue();
			rVar = ((SettingsModelString) m_rightVarMdl.createCloneWithValidatedValue(settings))
					.getStringValue();
		} catch (InvalidSettingsException e) {
			LOGGER.debug("Settings models could not be cloned with given " + "settings!");
		} finally {

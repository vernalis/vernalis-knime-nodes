...
public class ReportOptionsDialog2 extends JPanel {
	...
	public ReportOptionsDialog2(PdbConnectorConfig2 config) {
		...
		for (ReportCategory2 category : categories) {
			if (!category.isHidden()) {
				ReportCategoryDialog2 reportDlg = new ReportCategoryDialog2(category);
				super.add(reportDlg);
	...
	public final void loadSettingsFrom(final NodeSettingsRO settings, final PortObjectSpec[] specs)
		...
		try {
			updateComponent(settings.getString(AbstractPdbConnectorNodeModel.STD_REPORT_KEY));
		} catch (InvalidSettingsException e) {
			logger.warn(
					"Error loading string for key " + AbstractPdbConnectorNodeModel.STD_REPORT_KEY);

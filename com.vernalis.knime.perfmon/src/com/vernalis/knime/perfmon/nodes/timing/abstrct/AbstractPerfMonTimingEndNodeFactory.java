 ...
 *
 * @param <T>
 *            The Type of the NodeModel
 */
public abstract class AbstractPerfMonTimingEndNodeFactory
		extends NodeFactory<AbstractPerfMonTimingEndNodeModel> {
	...
	public NodeView<AbstractPerfMonTimingEndNodeModel> createNodeView(
			int viewIndex, AbstractPerfMonTimingEndNodeModel nodeModel) {
		return null;
	}
	...
	public AbstractPerfMonTimingEndNodeModel createNodeModel() {
		return isMemMon()
				? new AbstractMemMonPerfLoopEndNodeModel(getPortType(),
						getNumPorts())
				: new AbstractPerfMonTimingEndNodeModel(getPortType(),
						getNumPorts());
	}
	...
	protected NodeDescription createNodeDescription()
		...
		NodeDescription retVal = new NodeDescription() {
			...
			public String getIconPath() {
				return isMemMon() ? "Benchmarking_End_Mon.png"
						: "Benchmarking_End.png";
			}
			...
			public String getOutportDescription(int index) {
				...
				default:
					int modifiedIndex = index - (isMemMon() ? 3 : 2);
					...
					if (modifiedIndex < names.length) {
						return "Unmodified " + names[modifiedIndex] + " "
								+ getPortTypeDescription()
								+ " from final iteration";
					}
					return "Unmodified " + modifiedIndex + "th "
							+ getPortTypeDescription()
							+ " from final iteration";
				}
			...
			public Element getXMLDescription() {
				...
				node.setShortDescription("Loop end for execution timing"
				...
				introCursor.insertChars(
						"This node provides the end of a timing block for monitoring "
				...
				introCursor.insertElementWithText("p", "The incoming "
						+ getPortTypeDescription()

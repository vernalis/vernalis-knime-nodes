 ...
 */
public class AbstractMultiPortTimedLoopEndNodeModel extends NodeModel implements
		LoopEndNode {

	// the logger instance
	/** The Constant logger. */
	private static final NodeLogger logger = NodeLogger
			.getLogger(AbstractMultiPortTimedLoopEndNodeModel.class);
	...
	public AbstractMultiPortTimedLoopEndNodeModel(final int numPorts) {
		...
		super(FlowControlHelpers.createTimedEndOptionalInPorts(
				BufferedDataTable.TYPE, numPorts), FlowControlHelpers
				.createTimedEndPorts(BufferedDataTable.TYPE, numPorts));
		m_NumPorts = numPorts;
		m_resultContainer = new BufferedDataContainer[numPorts + 1];
	...
	public AbstractMultiPortTimedLoopEndNodeModel(final int numPorts,
		...
		super(
				(optionalInPorts) ? FlowControlHelpers
						.createTimedEndOptionalInPorts(BufferedDataTable.TYPE,
								numPorts)
						: FlowControlHelpers.createTimedEndInPorts(
								BufferedDataTable.TYPE, numPorts),
				FlowControlHelpers.createTimedEndPorts(BufferedDataTable.TYPE,
						numPorts));
		m_NumPorts = numPorts;
		m_resultContainer = new BufferedDataContainer[numPorts + 1];
		m_settings = new AbstractMultiPortLoopEndSettings(numPorts);
	...
	protected PortObject[] execute(final PortObject[] inData,
		...
		if (loopStartModel.terminateLoop()) {
			...
			outTables[m_NumPorts] = loopStartModel.getUnprocessedRows(exec);
	...
	protected PortObjectSpec[] configure(final PortObjectSpec[] inSpecs)
		...
		// Now need to check that the loop start is a timed loop start
		if (this.getLoopStartNode() instanceof AbstractTimedLoopStartNodeModel) {
			m_specs[m_NumPorts] = ((AbstractTimedLoopStartNodeModel) this

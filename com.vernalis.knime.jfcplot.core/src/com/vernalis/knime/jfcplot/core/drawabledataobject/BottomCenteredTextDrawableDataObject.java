...
/**
 * A {@link DrawableDataObject} to add a text label to a plot, with the bottom
 * centre of the text at the coordinates (x,y)
 * 
 * @author s.roughley
 *
 */
public class BottomCenteredTextDrawableDataObject
		implements DrawableDataObject {
	 ...
	 *            The text to draw
	 */
	public BottomCenteredTextDrawableDataObject(double x, double y,
			String text) {
		this(x, y, text, null);
	}
	 ...
	 *            colour
	 */
	public BottomCenteredTextDrawableDataObject(double x, double y, String text,
			Color colour) {
	...
	public void draw(Graphics2D g2, Rectangle2D dataArea, ValueAxis xAxis,
		...
		g2.drawString(text,
				(float) (xAxis.valueToJava2D(x, dataArea, RectangleEdge.BOTTOM)
						- fm.stringWidth(text) / 2.0),
				(float) yAxis.valueToJava2D(y, dataArea, RectangleEdge.LEFT));

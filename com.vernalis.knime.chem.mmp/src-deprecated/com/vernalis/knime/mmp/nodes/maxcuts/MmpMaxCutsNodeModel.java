...
public class MmpMaxCutsNodeModel extends NodeModel {
	...
	protected final SWIGObjectGarbageCollector gc =
	...
	private ColumnRearranger createColumnRearranger(DataTableSpec inSpec) {
		...
		DataColumnSpec newColSpec = new DataColumnSpecCreator(
				DataTableSpec.getUniqueColumnName(inSpec, "Max Number of Cuts"),
				IntCell.TYPE).createSpec();

		final int molColIdx =
				inSpec.findColumnIndex(m_molColName.getStringValue());
		rearranger.append(
				new SingleCellFactory(true, numThreads, queueSize, newColSpec) {
					...
					public DataCell getCell(DataRow row) {
						...
						ROMol mol = null;
						try {
							mol = gc.markForCleanup(
									getROMolFromCell(row.getCell(molColIdx)),
									currentWaveID);
						} catch (RowExecutionException e) {
							return retVal;
						}

						int maxCuts = RDKitFragmentationUtils.maxNumCuts(mol,
								bondMatch, m_AddHs.getBooleanValue(),
								m_allowTwoCutsBondValue.getBooleanValue());
	...
	protected DataTableSpec[] configure(DataTableSpec[] inSpecs)
		...
		bondMatch = gc.markForCleanup(
				RWMol.MolFromSmarts(fragSMIRKS.split(">>")[0]), 1);

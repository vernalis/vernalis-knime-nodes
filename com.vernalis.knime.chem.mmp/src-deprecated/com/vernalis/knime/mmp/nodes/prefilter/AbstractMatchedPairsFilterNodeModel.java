...
public class AbstractMatchedPairsFilterNodeModel extends NodeModel {
	...
	public AbstractMatchedPairsFilterNodeModel(int outPorts) {
		...
		prefStore.addPropertyChangeListener(new IPropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent event) {
				...
				m_verboseLogging = prefStore.getBoolean(
	...
	protected BufferedDataTable[] execute(BufferedDataTable[] inData,
		...
		MultiThreadWorker<DataRow, Boolean> processor =
				new MultiThreadWorker<DataRow, Boolean>(queueSize, numThreads) {
					...
					@Override
					protected Boolean compute(DataRow in, long index)
							throws Exception {
		...
		m_Logger.info("Checked Row " + rowCnt + " of " + numRows + "; Passed "
				+ keptRowCnt + " rows");
		return retVal;
	...
	protected static boolean processRow(DataRow row, int molIdx,
		...
		if (roMol == null || "".equals(roMol.MolToSmiles(true))) {
			// Log the failed row
			if (verboseLogging) {
				logger.info("No molecule found (Row: "
		...
		if (roMol.MolToSmiles().contains(".")) {
			// Log the failed row
			if (verboseLogging) {
				logger.info("Multicomponent molecule (Row: "
						+ row.getKey().getString() + ")");
			}
			return false;
		}

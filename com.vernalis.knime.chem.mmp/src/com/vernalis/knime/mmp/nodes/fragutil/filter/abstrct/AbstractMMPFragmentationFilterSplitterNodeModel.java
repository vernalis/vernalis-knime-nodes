...
public class AbstractMMPFragmentationFilterSplitterNodeModel<T, U>
	...
	protected BufferedDataTable[] doExecute(BufferedDataTable[] inData,
		...
		final int molColIdx = inData[0].getSpec()
				.findColumnIndex(m_molColName.getStringValue());
		...
		this.execute(inputRow, keeps, drops, inTable.size(), exec, bondMatch,
				maxNumCuts, addHs, molColIdx);
		return (isSplitter)
	...
	protected void execute(final RowInput inRow, RowOutput keeps,
			RowOutput drop, final long numRows, final ExecutionContext exec,
			U bondMatch, int maxNumCuts, boolean addHs, int molColIdx)
		...
		return new StreamableOperator() {
			...
			public void runFinal(PortInput[] inputs, PortOutput[] outputs,
				...
				final U bondMatch = fragUtilityFactory.getMatcher(
						fragSMIRKS.contains(">>") ? fragSMIRKS.split(">>")[0]
								: fragSMIRKS);
				...
				final boolean addHs =
						(maxNumCuts == 1) ? m_AddHs.getBooleanValue() : false;
				final int molColIdx = ((DataTableSpec) inSpecs[0])
						.findColumnIndex(m_molColName.getStringValue());

				// Run it - dont know row count!
				AbstractMMPFragmentationFilterSplitterNodeModel.this.execute(
						(RowInput) inputs[0], (RowOutput) outputs[0],
						isSplitter ? (RowOutput) outputs[1] : null, -1L, exec,
						bondMatch, maxNumCuts, addHs, molColIdx);
			}

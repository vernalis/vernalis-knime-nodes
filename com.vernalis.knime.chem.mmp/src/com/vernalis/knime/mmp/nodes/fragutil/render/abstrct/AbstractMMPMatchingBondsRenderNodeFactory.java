...
/**
 * Abstract NodeFactory class for the Render Matching bonds nodes
 * 
 * @author s.roughley
 * @param <T>
 ...
 */
public class AbstractMMPMatchingBondsRenderNodeFactory<T, U>
	 ...
	 */
	public AbstractMMPMatchingBondsRenderNodeFactory(
	...
	protected NodeDescription createNodeDescription()
		...
		try {
			return new AbstractMMPMatchingBondsRenderNodeDescription<>(
	...
	public AbstractColumnRearrangerFragmentationFactoryNodeModel<T, U> createNodeModel() {
		try {
			return new AbstractMMPMatchingBondsRenderNodeModel<>(
					fragUtilsFactory.getConstructor().newInstance());
		} catch (InstantiationException | IllegalAccessException | IllegalArgumentException
				| InvocationTargetException | NoSuchMethodException | SecurityException e) {
			throw new RuntimeException("Error instantiating Fragmentation Utilities factory", e);
	...
	protected NodeDialogPane createNodeDialogPane() {
		try {
			return new AbstractMMPMatchingBondsRenderNodeDialog<>(
					fragUtilsFactory.getConstructor().newInstance(),
					"Matching bond highlight colour", false, false);
		} catch (InstantiationException | IllegalAccessException | IllegalArgumentException
				| InvocationTargetException | NoSuchMethodException | SecurityException e) {
			throw new RuntimeException("Error instantiating Fragmentation Utilities factory", e);
		}

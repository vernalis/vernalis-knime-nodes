 ...
 * 
 */
public class ByteVectorAddRemainderOperator extends AggregationOperator {
	SparseByteVector sbv = null;
	DenseByteVector dbv = null;
	DataType colType;

	public ByteVectorAddRemainderOperator() {
		this(new OperatorData("ByteVector Add (Remainder)",
				"ByteVector Add (Remainder)", "ByteVector Add (Remainder)",
				false, true, ByteVectorValue.class, false),
				GlobalSettings.DEFAULT,
				OperatorColumnSettings.DEFAULT_EXCL_MISSING);
	}

	public ByteVectorAddRemainderOperator(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		this(new OperatorData("ByteVector Add (Remainder)",
				"ByteVector Add (Remainder)", "ByteVector Add (Remainder)",
				false, true, ByteVectorValue.class, false), globalSettings,
				opColSettings);
	}

	public ByteVectorAddRemainderOperator(OperatorData operatorData,
	...
	public String getDescription() {
		return "Adds counts in aggregated fingerprints.  "
				+ "When a count total exceeds 255, it"
				+ " is replaced with the remainder of the count / 255";
	}
	...
	public AggregationOperator createInstance(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		return new ByteVectorAddRemainderOperator(getOperatorData(),
	...
	protected boolean computeInternal(DataCell cell) {
		...
		if (colType == DenseByteVectorCell.TYPE) {
			DenseByteVector dbv1 = ((DenseByteVectorCell) cell)
					.getByteVectorCopy();
			if (dbv == null) {
				dbv = dbv1;
			} else {
				dbv = dbv.add(dbv1, true);
			}
		} else {
			SparseByteVector sbv1 = ((SparseByteVectorCell) cell)
					.getByteVectorCopy();
			if (sbv == null) {
				sbv = sbv1;
			} else {
				sbv = sbv.add(sbv1, true);
			}
		}
		return false;
	}

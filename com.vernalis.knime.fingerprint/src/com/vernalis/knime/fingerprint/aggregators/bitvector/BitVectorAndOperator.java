...
/**
 * Aggregation operator to allow 'AND' of bitvectors
 * 
 * @author s.roughley
 * 
 */
public class BitVectorAndOperator extends AggregationOperator {
	SparseBitVector sbv = null;
	DenseBitVector dbv = null;
	DataType colType;

	public BitVectorAndOperator() {
		this(new OperatorData("Bitvector AND", "Bitvector AND", "AND", false, true,
				BitVectorValue.class, false), GlobalSettings.DEFAULT,
				OperatorColumnSettings.DEFAULT_EXCL_MISSING);
	}

	public BitVectorAndOperator(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		this(new OperatorData("Bitvector AND", "Bitvector AND", "AND", false, true,
				BitVectorValue.class, false), globalSettings, opColSettings);
	}

	public BitVectorAndOperator(OperatorData operatorData, GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
	...
	public String getDescription() {
		return "Combines Bitvector fingerprints in group using a logic AND operation";
	}
	...
	public AggregationOperator createInstance(GlobalSettings globalSettings,
			OperatorColumnSettings opColSettings) {
		return new BitVectorAndOperator(getOperatorData(), globalSettings, opColSettings);
	}

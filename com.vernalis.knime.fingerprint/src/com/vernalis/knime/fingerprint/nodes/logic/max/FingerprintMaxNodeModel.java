...
public class FingerprintMaxNodeModel extends AbstractDoubleFingerprintSingleOutNodeModel {

	public FingerprintMaxNodeModel() {
		super(false, false, true, true, null);
	}

	@Override
	protected String getResultColumnName() {
		return "MAX (" + m_firstColName.getStringValue() + ", " + m_secondColName.getStringValue()
				+ ")";
	}
	...
	protected DataCell getByteVectorResultCell(DataCell fp1, DataCell fp2)
		...
		if (m_fpType == DenseByteVectorCell.TYPE) {
			return DenseByteVectorCellFactory.max(bvv1, bvv2);
		} else {
			return SparseByteVectorCellFactory.max(bvv1, bvv2);
		}
	}

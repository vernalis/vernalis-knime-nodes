...
import com.vernalis.knime.core.memory.MemoryUtils;
 ...
 * @author
 */
public class RunHeavyGCNodeModel extends NodeModel {

	private final NodeLogger m_logger =
			NodeLogger.getLogger(RunHeavyGCNodeModel.class);

	SettingsModelIntegerBounded m_numGCs =
			RunHeavyGCNodeDialog.createCollectionsNumberModel();
	SettingsModelIntegerBounded m_gcDelay =
			RunHeavyGCNodeDialog.createDelayModel();
	...
	protected PortObject[] execute(PortObject[] inObjects,
		...
		try {
			MemoryUtils.runHeavyGC(m_numGCs.getIntValue(),
					m_gcDelay.getIntValue());
		} catch (InterruptedException e) {
			m_logger.warn("Heavy GC incomplete due to InterruptedException: "
					+ e.getMessage());
		}
	...
	private void reportMemoryStatus(boolean reportTotalMemory) {
		...
		if (reportTotalMemory) {
			m_logger.info(
					"Max. available memory: " + m_rt.maxMemory() / MB + " MB");
		}
	}
	...
	protected void saveSettingsTo(final NodeSettingsWO settings) {
		m_numGCs.saveSettingsTo(settings);
		m_gcDelay.saveSettingsTo(settings);
	}
	...
	protected void loadValidatedSettingsFrom(final NodeSettingsRO settings)
			throws InvalidSettingsException {
		m_numGCs.loadSettingsFrom(settings);
		m_gcDelay.loadSettingsFrom(settings);
	}
	...
	protected void validateSettings(final NodeSettingsRO settings)
			throws InvalidSettingsException {
		m_numGCs.validateSettings(settings);
		m_gcDelay.validateSettings(settings);
	}
